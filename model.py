...
def train_val_test_split(train_folder, validation_folder, test_folder):
    ...
    for class_folder in os.listdir(DST_DIR):
        ...
        if os.path.isdir(class_path):
            ...
            create_symlinks(class_path, os.path.join(train_folder, class_folder), train_set)
            create_symlinks(class_path, os.path.join(validation_folder, class_folder),val_set)
            create_symlinks(class_path, os.path.join(test_folder, class_folder), test_set)
...
def transform_datasets(train_mean, train_std):
    ...
    train_dataset = ImageFolder(root=os.path.join(OUTPUT_DIR, 'train'), transform=data_transforms)
    val_dataset = ImageFolder(root=os.path.join(OUTPUT_DIR, 'val'), transform=data_transforms)
    test_dataset = ImageFolder(root=os.path.join(OUTPUT_DIR, 'test'), transform=data_transforms)
...
def main():
    ...
    if (not os.path.exists(DST_DIR)):
        print(f"File directory data doesn't exists") 
        print(DST_DIR)
    ...
    train_dataloader, val_dataloader, test_dataloader = load_data_into_dataloaders(train_dataset, val_dataset, test_dataset, batch)
    ...
    model_18 = train_val(model=model_18, criterion=criterion, optimiser=optimiser, dataloaders=dataloaders, datasets=datasets)
    # Test model
    ...
    # Visual Separators 
    print() 
    print(f"Train model ResNet 152")
    print("-" * 100)
